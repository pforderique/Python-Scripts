...
class PatientDatabase(AVL):
    def __init__(self, item = None, parent = None):
        ...
        if self.item: 
            ...
            self.num_nodes = 0
            self.min = 10000000
            self.max = -1 # changed from 0 to None...? bc maybe min keeps at 0 all the way down
    ...
    def update(self):
        ...
        if self.left or self.right:                                          ## PROB HAS TO BE HERE. YOU ARE NOT UPDATING IN/MAX CORRECTLY? initialize too bruh
            left_min = self.left.min   if self.left  else self.right.min
            right_min = self.right.min if self.right else self.left.min
            left_max = self.left.max   if self.left  else self.right.max
            right_max = self.right.max if self.right else self.left.max

            self.min = min(left_min, right_min)
    ...
    def num_patients(self,p1,p2):
        ...
        # if self.min == self.max == -1: return 0 # adding this line will take us from overshooting to always 0
